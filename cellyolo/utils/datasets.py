...
class MatchImagesAndLabels(Dataset):  # for remove some unnecessary non normalized label file
    def __init__(self, currentpath,path, img_size=416, batch_size=16):
        ...
        pbar = tqdm(self.label_files, desc='Caching labels')
        nm, nf, ne, ns, nd = 0, 0, 0, 0, 0  # number missing, found, empty, datasubset, duplicate
        no_label_image = open('no_label_image.txt', 'w')
        for i, file in enumerate(pbar):
            try:
                with open(file, 'r') as f:
                    l = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)
                ...
                no_label_image.write(self.img_files[i]+'\n')
                continue
            if l.shape[0]:

                if ((l[:, 1:] <= 1).all()==False ):
                    print('non-normalized or out of bounds coordinate labels: %s' % self.img_files[i])
                    os.remove((str(currentpath + (self.label_files[i]))).replace("\\","/"))
                if np.unique(l, axis=0).shape[0] < l.shape[0]:  # duplicate rows
                    nd += 1
                    #print('WARNING: mutil object occurs in same image  in %s' % self.label_files[i])  # duplicate rows
                self.labels[i] = l
                nf += 1  # file found
            else:
                ne += 1
