...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    s = [len(unique_classes), tp.shape[1]]  # number of class, number iou thresholds (i.e. 10 for mAP0.5...0.95)
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            # ax.set_title('cellyolo'); ax.set_xlabel('Recall'); ax.set_ylabel('Precision')
            # ax.set_xlim(0, 1)
            # fig.tight_layout()
...
def drawprecisionrecall(recall, precision):
    fig, ax = plt.subplots(1, 1, figsize=(4, 4))
    print("成功")
    ax.plot(recall, precision, color='green', label='Precision-Recall Curve', marker='.')
    ax.set_title('CellYolo_PR__train');
    ...
    fig.tight_layout()
    fig.savefig('Precision-Recall Curve_train.png', dpi=300)

    # Arguments
    #    recall:    The recall curve (list).
    #    precision: The precision curve (list).
    # Returns
    #    recall precison graph
...
def GBCIOU(box1, box2, x1y1x2y2=True):
    # For two object prediction overlapping case
    box2 = box2.t()
    # Get the coordinates of bounding boxes
    if x1y1x2y2:  # x1, y1, x2, y2 = box1
        b1_x1, b1_y1, b1_x2, b1_y2 = box1[0], box1[1], box1[2], box1[3]
    ...
    w1, h1 = b1_x2 - b1_x1, b1_y2 - b1_y1
    w2, h2 = b2_x2 - b2_x1, b2_y2 - b2_y1
    union = (w1 * h1 + 1e-16) + w2 * h2 - inter  # 这个算的是面积
    # iou
    iou = inter / union
    cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
    ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
    # Generalized IoU https://arxiv.org/pdf/1902.09630.pdf
    # the smallest enclosing convex object area
    enclose_area = cw * ch + 1e-16  # convex area
    giou = iou - (enclose_area - union) / enclose_area

    if giou >= 0.1:
        box1[0], box1[1], box1[2], box1[3] = torch.min(b1_x1, b2_x1), torch.min(b1_y1, b2_y1), torch.max(b1_x2,
                                                                                                         b2_x2), torch.max(
            b1_y2, b2_y2)
        box2[0], box2[1], box2[2], box2[3] = torch.min(b1_x1, b2_x1), torch.min(b1_y1, b2_y1), torch.max(b1_x2,
                                                                                                         b2_x2), torch.max(
            b1_y2, b2_y2)
    else:
        # because the input is already sorted with the confidence, so if it not overlapping then we output the most confident object and avoid of second mask interruption
        box2[0], box2[1], box2[2], box2[3] = b1_x1, b1_y1, b1_x2, b1_y2
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    if red == 'sum':
        ...
        #this will cause the break of division by zero if there is no labels occcurs
        # lbox *= 3 / ng
    ...
    # the loss is the combination of the the differences between the ground truth and objectness score
...
def plot_one_cycle(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding cycle on image img
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    imgcopy = np.zeros(img.shape, dtype=np.uint8)
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.circle(imgcopy,
               (int(abs(int(x[2]) - int(x[0])) / 2) + int(x[0]), int(abs(int(x[3]) - int(x[1])) / 2) + int(x[1])),
               int(min(abs(int(x[2]) - int(x[0])), abs(int(x[3]) - int(x[1]))) / 2), (255, 0, 0), thickness=-1)
    # cv2.imwrite('data/imgcopy.jpg',imgcopy)
    img_rgb = cv2.cvtColor(imgcopy, cv2.COLOR_BGR2RGB)  # change the colorspcae of the image from BGR to RGB
    img_hsv = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2HSV)  # change the color space of image from RGB to HSV
    # set the threshold to select the spots
    hsv_low = (0, 255, 255)  # lower threshold
    hsv_high = (240, 255, 255)  # higher threshold

    cyclemask = cv2.inRange(img_hsv, hsv_low, hsv_high)  # create a binary mask using the threshold
    # cv2.imwrite('imgmask.jpg',cyclemask)

    img = cv2.bitwise_and(img, img, mask=cyclemask)  # use the binary mask to extact only the hotspots from the image
    # cv2.imwrite('img.jpg',img)
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line thickness
    color = color or [random.randint(0, 255) for _ in range(3)]

    '''
    arealefttriangle = np.array([[0, 0], [int(x[0]), 0], [0, img.shape[0]], ])
    arealefttriangle2 = np.array([[int(x[0]), img.shape[1]],[int(x[0]), 0], [0, img.shape[0]]])
    arearighttriangle = np.array([[img.shape[1], 0], [int(x[2]), img.shape[0]],[img.shape[1], img.shape[0]]])
    arearighttriangle2 = np.array([[int(x[2]), 0], [img.shape[1], 0], [int(x[2]), img.shape[0]]])
    areaupbox = np.array([[int(x[0]), 0], [int(x[0]), int(x[1])], [int(x[2]), int(x[1])], [int(x[2]), 0]])
    areaupperbox = np.array([[int(x[0]), int(x[3])],[int(x[0]), img.shape[1]], [int(x[2]),img.shape[1]], [int(x[2]), int(x[3])]])

    cv2.fillPoly(img, [ arealefttriangle,arealefttriangle2,arearighttriangle2,arearighttriangle,areaupbox, areaupperbox ], (255, 255, 255))
    '''
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, (255, 255, 255), -1)  # filled
...
def findEllipses(imgpath):
    img = cv2.imread(imgpath)
    imggray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # contours, _ = cv2.findContours(img.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    contours, _ = cv2.findContours(imggray.copy(), mode=cv2.RETR_LIST, method=cv2.CHAIN_APPROX_SIMPLE)
    # print(contours)
    ellipseMask = np.zeros(imggray.shape, dtype=np.uint8)
    contourMask = np.zeros(imggray.shape, dtype=np.uint8)
    ...
    for i, contour in enumerate(contours):
        ...
        circularity = (pi_4 * area) / (arclen * arclen)  # 圆
        print('circularity')
        ...
        if circularity > 0.6:
            cv2.fillPoly(ellipseMask, [poly], (255, 255, 255))
            continue
        ...
        similarity = cv2.matchShapes(poly.reshape((poly.shape[0], 1, poly.shape[1])), contour,
                                     cv2.cv.CV_CONTOURS_MATCH_I2, 0)
        if similarity <= 0.2:
            cv2.fillPoly(contourMask, [poly], (0, 0, 0))
...
def plot_results_overlay(start=0, stop=0):  # from utils.utils import *; plot_results_overlay()
    ...
    t = ['GIoU', 'Objectness', 'Classification', 'P-R_train', 'mAP-F1']  # titles
...
def plot_results(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
    ...
    recall = []
    precision = []
    s = ['GIoU_train', 'Objectness_train', 'Classification_train', 'Precision_train', 'Recall_train',
    ...
    for f in sorted(files):
        ...
        x = range(start, min(stop, n) if stop else n)
        for i in range(10):
            recall.append(results[3, x])
            precision.append(results[4, x])
            y = results[i, x]
            if i in [0, 1, 2, 5, 6, 7]:
                y[y == 0] = np.nan  # dont show zero loss values
    ...
    # drawprecisionrecall(recall,precision)
    fig.tight_layout()
    ax[1].legend()
