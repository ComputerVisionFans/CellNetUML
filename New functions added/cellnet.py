...
predictionlabel8, targetlabel8,pathlist8 = [], [], []
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.nextbutton7.clicked.connect(lambda: self.nextbuttonClick(self.nextbutton7))
        self.nextbutton8.clicked.connect(lambda: self.nextbuttonClick(self.nextbutton8))
        ...
        self.previousbutton8.clicked.connect(lambda: self.perviousbuttonClick(self.previousbutton8))
        ...
        # skincancerdataset page activated
        self.skincancerdataset.clicked.connect(lambda: self.skincancerdatasetClick(self.skincancerdataset))
        ...
        # Compare with all state of the arts algorithm on Skin Cancer datasets
        self.compareonskin.clicked.connect(lambda: self.compareonskinClick(self.compareonskin))
        ...
        # check the pic upload first
        self.inputbutton8.clicked.connect(lambda: self.inputbutton8Click(self.inputbutton8))
        # make a prediction on Skin Cancer
        self.prediction8.clicked.connect(lambda: self.prediction8Click(self.prediction8))
        ...
        #slider on Skin Cancer page
        self.slider8.valueChanged.connect(self.valuechangeofslider8)
    ...
    # skincancerdataset page activated
    def skincancerdatasetClick(self, skincancerdataset):
    ...
    #needs to add the real Skin Cancer comparison image later!!
    def compareonskinClick(self, compareonskin):
        self.hins8.hide()
        rel_path = "./paperimage/"
        paperimage = os.path.join(script_dir, rel_path)
        ...
        pixmap = QPixmap(valimage[0])
        self.imgdata8.show()
        self.val4_4.setValue(0.767860)
        pixmap_resized = pixmap.scaled(self.imgdata8.width(), self.imgdata8.height(), QtCore.Qt.KeepAspectRatio)
        self.imgdata8.setPixmap(pixmap_resized)
    ...
    def valuechangeofslider8(self):
        global pathlist8, predictionlabel8, targetlabel8
        size = int(self.slider8.value())
        print(size)
        print("pathlist:",pathlist8)
        print("pathlist:",len(pathlist8))
        rel_path = "./ISIC2019_infos/"
        imgfolderpathofskincancer = os.path.join(script_dir, rel_path)
        print("predictionlabel:", predictionlabel8)
        print("predictionlabel:", len(predictionlabel8))
        imgfolderpath = str(self.savepath8.toPlainText())

        if self.inputbutton8.isChecked()==True:
            if  pathlist8:
                self.hins8.show()
                self.hins8.setText(str(pathlist8[int((size / 100) * len(pathlist8))]))
                print(int((size / 100) * len(pathlist8)))
                pixmap = QPixmap((pathlist8[int((size / 100) * len(pathlist8))]))
                pixmap_resized = pixmap.scaled(self.imgdata8.width(), self.imgdata8.height(), QtCore.Qt.KeepAspectRatio)

                self.imagelabel8.setText(str(labels8classesofSkinCancer[targetlabel8[int((size / 100) * len(pathlist8))]]))
                #print(int((size / 100) * len(targetlabel8)))
                #self.predictiontype8.setText(str(predictionlabel8[int((size / 100) * len(predictionlabel8))]))
                self.predictiontype8.setText(str(labels8classesofSkinCancer[predictionlabel8[int((size / 100) * len(pathlist8))]]))
                #print(int((size / 100) * len(predictionlabel8)))
                ...
                print(my_pictures)
                if not len(my_pictures) == 0:
                    # self.imagelabel8.setText(str(targetlabel8[0]))
                    # self.predictiontype8.setText(str(predictionlabel8[0]))
                    self.hins8.show()
                    self.hins8.setText(str(my_pictures[int((size / 100) * len(my_pictures))]))
                    pixmap = QPixmap((my_pictures[int((size / 100) * len(my_pictures))]))
                    pixmap_resized = pixmap.scaled(self.imgdata8.width(), self.imgdata8.height(),
                                                   QtCore.Qt.KeepAspectRatio)
                    self.imgdata8.setPixmap(pixmap_resized)
                else:
                    box = QtWidgets.QMessageBox()
            ...
            my_picturesoftemplate = _globrec(str(imgfolderpathofskincancer), '\*')
            print(len(my_picturesoftemplate))
            print(my_picturesoftemplate[0][0])
            self.hins8.show()
            self.hins8.setText("Because you didn't upload image,so you see default dataset of the skin cancer!")
            self.imgdata8.show()
            pixmap = QPixmap((my_picturesoftemplate[int((size / 100) * len(my_picturesoftemplate))]))
            pixmap_resized = pixmap.scaled(self.imgdata8.width(), self.imgdata8.height(),
                                               QtCore.Qt.KeepAspectRatio)
    ...
    def inputbutton8Click(self,inputbutton8):
        ...
        if  imgfolderpath == '':
            ...
            box = QtWidgets.QMessageBox()
            box.warning(self, "Hello", "Please input Image Folder and  then click upload button!")
            self.inputbutton8.setChecked(False)
        else:
            self.hins8.hide()
            my_pictures = _globrec(str(imgfolderpath), '\*')
            infostring1 = "Succeed received " + str(len(my_pictures)) + " images! Start test other functions "
    ...
    def prediction8Click(self,prediction8):
        imgfolderpath = str(self.savepath8.toPlainText())
        global pathlist8, predictionlabel8, targetlabel8

        if self.inputbutton8.isChecked():
            my_pictures = _globrec(str(imgfolderpath), '\*')
            infostring1 = "Succeed received " + str(len(my_pictures)) + " images! it takes " +str(len(my_pictures)*0.38 )+  " second for prediction with CellNet."
            box = QtWidgets.QMessageBox()
            box.information(self, "Hello", infostring1)
            ...
            parser = argparse.ArgumentParser()
            parser.add_argument('--weights', type=str, default='ISIC2019_infos/model_best.pt',
                                help='path to weights file')
            parser.add_argument('--source', type=str, default=imgfolderpath, help='source')  # input file/folder, 0 for webcam
            parser.add_argument('--img-size', type=int, default=416, help='inference size (pixels)')
            parser.add_argument('--device', default='', help='device id (i.e. 0 or 0,1) or cpu')
            ...
            print(opt)
            with torch.no_grad():
                predictionlabel8,targetlabel8,Accuracy,pathlist8=detect8skincancer(opt)
                info="Succeed evaluated on " + str(len(predictionlabel8)) \
                     + " val images! it takes " +str(len(predictionlabel8)*0.38 )+  " second for prediction with CellNet."
                self.hins8.show()
                self.hins8.setText(info)
                infotext=info+" Accuracy:"+str(round(Accuracy, 2))
                self.hins8.setText(infotext)
